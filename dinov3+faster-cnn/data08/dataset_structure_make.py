...
# 设置data08的绝对路径
base_dir = r"D:\Study\VascularStenosis\CNN+LSTM\dataset\data08"
output_json_file = os.path.join(base_dir, "data08_structure.json")
            ...
            if os.path.exists(images_folder) and os.path.exists(annotations_folder):
                ...
                for image_file in os.listdir(images_folder):
                    if image_file.endswith(".jpg"):
                        image_path = os.path.join("data08", num1_folder, "images", image_file)
                        data_structure[num1_folder]["images"].append(image_path)
                
                # 遍历 annotations 文件夹并添加所有 xml 文件的相对路径
                for annotation_file in os.listdir(annotations_folder):
                    if annotation_file.endswith(".xml"):
                        annotation_path = os.path.join("data08", num1_folder, "annotations", annotation_file)
