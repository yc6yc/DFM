...
def _sort_case_ids(case_ids):
    ...
    return sorted(case_ids, key=_key)


def convert_custom_json_to_coco(structure_json_path, output_dir, train_case_count=67, val_case_count=17):
    ...
    # 2. 获取所有病例 ID（按数值升序固定排序）
    case_ids = _sort_case_ids(list(raw_data.keys()))

    # 3. 按病例固定划分训练集和验证集（前67训练，后17验证）
    needed = train_case_count + val_case_count
    if len(case_ids) < needed:
        raise RuntimeError(
            f"病例数量不足，至少需要 {needed} 例，当前仅 {len(case_ids)} 例"
        )

    train_cases = case_ids[:train_case_count]
    val_cases = case_ids[train_case_count:train_case_count + val_case_count]

    print(f"总病例数: {len(case_ids)}")
    print(f"训练集病例数: {len(train_cases)}")
    print(f"验证集病例数: {len(val_cases)}")
    print(f"训练集病例ID范围: {train_cases[0]} -> {train_cases[-1]}")
    print(f"验证集病例ID范围: {val_cases[0]} -> {val_cases[-1]}")
...
if __name__ == "__main__":
    # 替换为你实际的文件名
    current_json = "data08_structure.json"
    # 输出结果的文件夹
